...
This ~dotfiles~ project is defined as git bare repository. You can ensure your dotfiles by similar way.
See the usage concept.
In this case, the git bare repository is cloned to the ~~/.dotfiles~ and the existing files which
are already included in the bare are moved to the ~.config-backup~ directory.
...
Just for information, updates/initial scripts are placed in ~~/bin~ directory.
*** ~init-git-subtrees-dotfiles.sh~
*** ~pull-git-subtrees-dotfiles.sh~
#+BEGIN_SRC bash
#!/bin/bash
function config {
   /usr/bin/git --git-dir=$HOME/.dotfiles/ --work-tree=$HOME $@
