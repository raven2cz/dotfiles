...
function serializeChildren(root) {
  ...
  for (let i = 0, len = root.owned.length; i < len; i++) {
    ...
    result[node.name] = { ...serializeValues(node.sourceMap),
  ...
  get(target, property, receiver) {
    ...
    if (Listener && (typeof value !== "function" || target.hasOwnProperty(property))) {
      ...
      if (wrappable && (nodes = getDataNodes(value))) {
        node = nodes._ || (nodes._ =  createSignal(undefined, false, {
          internal: true
        }) );
        node[0]();
      }
      nodes = getDataNodes(target);
      node = nodes[property] || (nodes[property] =  createSignal(undefined, false, {
        internal: true
      }) );
      node[0]();
    }
...
function createState(state, options) {
  ...
  const wrappedState = wrap(unwrappedState,  (options && options.name || hashValue(unwrappedState)), true);
  {
...
function createResourceState(state, options = {}) {
  ...
  const resourceTraps = {
    get(target, property, receiver) {
      ...
      return wrappable ? wrap(value,  options.name && `${options.name}:${property}`) : value;
    },
    set() {
      return true;
    },
  ...
  const unwrappedState = unwrap(state || {}, true),
        wrappedState = wrap(unwrappedState,  (options && options.name || hashValue(unwrappedState)), true, resourceTraps);
  {
    const name = options && options.name || hashValue(unwrappedState);
    registerGraph(name, {
      value: unwrappedState
...
const proxyTraps$1 = {
  get(target, property, receiver) {
    ...
    return wrappable ? wrap(value,  target[$NAME] && `${target[$NAME]}:${property}`, false, proxyTraps$1) : value;
  },
...
function createMutable(state, options) {
  const unwrappedState = unwrap(state || {}, true);
  const wrappedState = wrap(unwrappedState,  (options && options.name || hashValue(unwrappedState)), true, proxyTraps$1);
  {
    const name = options && options.name || hashValue(unwrappedState);
    registerGraph(name, {
      value: unwrappedState
