...
function createHTML(r, {
  ...
  let uuid = 1;
  r.delegate = (el, ev, expr) => {
    if (Array.isArray(expr)) {
      el[`__${ev}`] = expr[0];
      el[`__${ev}Data`] = expr[1];
    } else el[`__${ev}`] = expr;
  };
  r.wrapProps = props => {
  ...
  function parseKeyValue(tag, name, isSVG, isCE, options) {
    ...
    if (name === "style") {
      ...
      const id = uuid++;
      options.exprs.push(`const v${id} = ${expr}`, `for (const e in v${id}) ${tag}.addEventListener(e, v${id}[e])`);
    } else if (name === "onCapture") {
      const id = uuid++;
      options.exprs.push(`const v${id} = ${expr}`, `for (const e in v${id}) ${tag}.addEventListener(e, v${id}[e], true)`);
  ...
  function processComponent(node, options) {
    ...
    if (options.parent) options.exprs.push(`r.insert(${options.parent}, r.createComponent(exprs[${componentIdentifier}], r.wrapProps({${props.join(", ") || ""}}))${tag ? `, ${tag}` : ""})`);else options.exprs.push(`${options.fragment ? "" : "return "}r.createComponent(exprs[${componentIdentifier}], r.wrapProps({${props.join(", ") || ""}}))`);
    options.path = tag;
    options.first = false;
  }
...
var index = createHTML({
  ...
  NonComposedEvents: web.NonComposedEvents,
