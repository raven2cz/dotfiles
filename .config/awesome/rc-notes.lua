...
iconCPU = wibox.widget.imagebox(beautiful.widget_cpu)
textCPU = wibox.widget.background(lain.widgets.cpu({
    timeout = 10,
    settings = function()
        widget:set_markup("" .. cpu_now.usage .. " % ")
    end }), "#313131" )

iconRAM = wibox.widget.imagebox(beautiful.widget_mem)    
textRAM = lain.widgets.mem({
    timeout = 30,
    settings = function()
        widget:set_markup("" .. mem_now.rel .. " % - " .. mem_now.used .. " MB ")
    end })    
    
graphCPU = awful.widget.graph()
graphCPU:set_width(50)
graphCPU:set_background_color("#313131")
...
statusBAT = lain.widgets.bat({
    settings = function()
        if bat_now.status == "Full " then
            widget:set_markup("Full-100 % ")
        elseif tonumber(bat_now.perc) >= 100 then
            widget:set_markup("" .. bat_now.status .. "-100 % ")
        else
            widget:set_markup("" .. bat_now.status .. "-" .. bat_now.perc .. " % ")
        end
    end })

iconNET = wibox.widget.imagebox(beautiful.widget_net)    
statusNET = wibox.widget.background(lain.widgets.net({
    timeout = 30,
    settings = function()
        widget:set_markup("" .. net_now.state .. " ")
    end }), "#313131" )
...
togglebox = awful.widget.launcher({ image = beautiful.awesome_icon,
                                    command = "" })
...
for s = 1, screen.count() do
    ...
    right_layout:add(iconBAT)
    right_layout:add(statusBAT)
    right_layout:add(arrl_ld)    
    right_layout:add(iconCPU)    
    right_layout:add(textCPU)
    right_layout:add(graphCPU)
    right_layout:add(arrl_dl)    
    right_layout:add(iconRAM)        
    right_layout:add(textRAM)
    right_layout:add(arrl_ld)    
    right_layout:add(iconNET)        
    right_layout:add(statusNET)
    right_layout:add(arrl_dl)    
    right_layout:add(spacer)    
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ altkey ,"Control"}, "space", function()  exec(command.gexec) end),   
